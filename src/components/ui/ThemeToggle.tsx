...
export default function ThemeToggle() {
  ...
  return (
    ...
    >
      {isDarkMode ? (
        <Moon className="h-3.5 w-3.5 md:h-4 md:w-4 text-white transition-all duration-300 ease-in-out motion-reduce:transition-none" />
      ) : (
        <Sun className="h-3.5 w-3.5 md:h-4 md:w-4 text-black/70 transition-all duration-300 ease-in-out motion-reduce:transition-none" />
      )}
    </Button>
  );
}
