...
export const TRANSFER_POLICY = Object.freeze({
  ...
  MAX_CHUNKS_PER_FILE: 1_000_000,
  MAX_REPAIR_ROUNDS: 5,
  MAX_DUPLICATE_BYTES: 50 * 1024 * 1024,
...
type TransferPolicy = typeof TRANSFER_POLICY;
...
const MSG_TYPE = {
  ...
  SESSION_POLICY_ACK: 'SESSION_POLICY_ACK',
  SESSION_POLICY_REJECT: 'SESSION_POLICY_REJECT',
...
export class WebRTCService {
  ...
  private agreedPolicy: TransferPolicy | null = null;
  private policyReady = false;
  private policyReadyPromise: Promise<void> | null = null;
  private resolvePolicyReady: (() => void) | null = null;
  private sessionTimeoutId: ReturnType<typeof setTimeout> | null = null;
  private declaredFileCount = 0;
  private declaredTotalBytes = 0;
  private startedFileCount = 0;
  private startedTotalBytes = 0;
  private receivedTotalBytes = 0;
  private indexedDbBytes = 0;
  private repairRoundsSent = new Map<number, number>();
  private repairRoundsReceived = new Map<number, number>();
  private limitViolationSent = false;
  ...
  private getActivePolicy(): TransferPolicy {
    return this.agreedPolicy ?? TRANSFER_POLICY;
  }

  private beginPolicyNegotiation(): void {
    this.policyReady = false;
    this.agreedPolicy = null;
    this.policyReadyPromise = new Promise<void>((resolve) => {
      this.resolvePolicyReady = resolve;
    });
    this.startSessionTimeout();
    if (this.role === 'sender') {
      this.sendControlMessage({ type: MSG_TYPE.SESSION_POLICY, policy: TRANSFER_POLICY });
    }
  }

  private markPolicyReady(policy: TransferPolicy): void {
    this.agreedPolicy = policy;
    this.policyReady = true;
    this.resolvePolicyReady?.();
    this.resolvePolicyReady = null;
  }

  private async waitForPolicyAgreement(): Promise<void> {
    if (this.policyReady) return;
    if (!this.policyReadyPromise) {
      throw new Error('Transfer policy negotiation has not started');
    }
    await Promise.race([
      this.policyReadyPromise,
      new Promise<never>((_, reject) => setTimeout(() => reject(new Error('Timed out waiting for transfer policy agreement')), 10000)),
    ]);
  }
  ...
  private startSessionTimeout(): void {
    ...
    this.sessionTimeoutId = setTimeout(() => {
      void this.enforceLimit('MAX_SESSION_DURATION', 'The transfer session exceeded the 2-hour duration limit.');
    }, TRANSFER_POLICY.MAX_SESSION_DURATION);
  }

  private validatePolicy(candidate: unknown): TransferPolicy | null {
    if (!candidate || typeof candidate !== 'object') return null;
    const value = candidate as Record<string, unknown>;
    const keys = Object.keys(TRANSFER_POLICY) as Array<keyof TransferPolicy>;
    for (const key of keys) {
      if (!Number.isSafeInteger(value[key]) || Number(value[key]) <= 0 || Number(value[key]) > TRANSFER_POLICY[key]) return null;
    }
    return { ...TRANSFER_POLICY, ...Object.fromEntries(keys.map((key) => [key, Number(value[key])] )) } as TransferPolicy;
  }

  private async enforceLimit(violation: string, detail: string): Promise<void> {
    if (this.limitViolationSent || this.isTerminalLifecycleState()) return;
    this.limitViolationSent = true;
    const message = `Transfer rejected: ${detail}`;
    try {
      this.sendControlMessage({ type: MSG_TYPE.LIMIT_EXCEEDED, violation, message });
    } catch (error) {
      console.warn('Unable to notify peer about transfer limit violation:', error);
    }
    this.onStatusMessage?.(message);
    this.onError?.(message);
    this.failTransfer(message, { cleanup: true, serverCancelReason: `limit-${violation.toLowerCase()}` });
    this.dataChannel?.close();
    this.binaryChannel?.close();
  }

  private validateFileMetadata(file: FileMetadata, totalCount: number, nextTotal: number): string | null {
    const policy = this.getActivePolicy();
    if (totalCount > policy.MAX_FILES) return `MAX_FILES exceeded (${totalCount} > ${policy.MAX_FILES})`;
    if (!Number.isSafeInteger(file.fileIndex) || file.fileIndex < 0 || file.fileIndex >= policy.MAX_FILES) return 'Invalid file index';
    if (!Number.isSafeInteger(file.size) || file.size < 0 || file.size > policy.MAX_FILE_BYTES) return `MAX_FILE_BYTES exceeded for ${file.name}`;
    if (nextTotal > policy.MAX_TOTAL_BYTES) return `MAX_TOTAL_BYTES exceeded`;
    if (typeof file.name !== 'string' || new TextEncoder().encode(file.name).byteLength > policy.MAX_FILE_NAME_BYTES) return 'File metadata name is too large';
    return null;
  }
  ...
  private async requestMissingChunksFromSender(
    ...
    const policy = this.getActivePolicy();
    const nextRound = (this.repairRoundsSent.get(fileIndex) ?? 0) + 1;
    if (nextRound > policy.MAX_REPAIR_ROUNDS) { void this.enforceLimit('MAX_REPAIR_ROUNDS', `Repair rounds exceeded the limit for file ${fileName}.`); return missingChunks; }
    this.repairRoundsSent.set(fileIndex, nextRound);
    for (let attempt = 1; attempt <= policy.MAX_REPAIR_ROUNDS && missingChunks.length > 0; attempt++) {
      console.warn(
  ...
  private describeChannelError(event: Event): string {
    // RTCDataChannel 'error' events carry an RTCError on `.error`, but a
    // plain console.error(event) stringifies to "[object RTCErrorEvent]"
    // and loses everything useful. Pull the real fields out instead.
    const rtcError = (event as RTCErrorEvent).error;
    if (!rtcError) return event.type || 'unknown data channel error';
    const parts = [
      rtcError.errorDetail,
      rtcError.message,
      rtcError.sctpCauseCode != null ? `sctpCauseCode=${rtcError.sctpCauseCode}` : null,
      rtcError.sdpLineNumber != null ? `sdpLine=${rtcError.sdpLineNumber}` : null,
    ].filter(Boolean);
    return parts.length > 0 ? parts.join(' | ') : 'unknown RTCError';
  }
  ...
  private setupDataChannelHandlers(): void {
    ...
    this.dataChannel.onopen = () => {
      ...
      this.beginPolicyNegotiation();
      this.onDataChannelOpen?.();
      if (this.role === 'sender' && this.filesToSend.length > 0) {
        setTimeout(() => { void this.startTransfer(); }, 100);
      }
    };
    ...
    this.dataChannel.onerror = (error) => {
      console.error('Data channel error:', this.describeChannelError(error));
    ...
    this.dataChannel.onmessage = async (event) => {
      try {
        ...
        if (typeof data === 'string') {
          if (data.length > TRANSFER_POLICY.MAX_CONTROL_MESSAGE_BYTES) {
            void this.enforceLimit('MAX_CONTROL_MESSAGE_BYTES', 'A control message exceeded the metadata size limit.');
            return;
          }
          if (/^\d+:\d+:\d+\|/.test(data)) {
  ...
  private setupBinaryChannelHandlers(): void {
    ...
    this.binaryChannel.onclose = () => {
      if (!this.transferCompleted) {
        this.onStatusMessage?.('Connection closed');
      }
    };

    this.binaryChannel.onerror = (error) => {
      console.error('Binary channel error:', this.describeChannelError(error));
      if (!this.transferCompleted) {
        this.failTransfer('Connection error occurred', {
          serverCancelReason: 'binary-channel-error',
        });
      }
    };
    ...
    this.binaryChannel.onmessage = (event) => {
      ...
      // Every binary chunk this app sends carries an 8-byte header
      // [fileIndex:4][chunkIndex:4] followed by the chunk payload.
      if (data.byteLength < 8) {
        console.error(`❌ Received malformed binary chunk: ${data.byteLength} bytes (expected an 8-byte header + payload)`);
        return;
      }
      ...
      console.log(`📦 Received binary chunk ${chunkIndex} for file ${fileIndex} (${chunkData.byteLength} bytes + 8 byte header)`);
  ...
  private async startTransfer(): Promise<void> {
    ...
    await this.waitForPolicyAgreement();
  ...
  private async handleControlMessage(message: ControlMessage): Promise<void> {
    if (message.type === MSG_TYPE.SESSION_POLICY) {
      const policy = this.validatePolicy(message.policy);
      if (!policy) {
        this.sendControlMessage({ type: MSG_TYPE.SESSION_POLICY_REJECT, message: 'Proposed transfer policy is unacceptable.' });
        await this.enforceLimit('SESSION_POLICY', 'The peer proposed unacceptable transfer limits.');
        return;
      }
      this.markPolicyReady(policy);
      this.sendControlMessage({ type: MSG_TYPE.SESSION_POLICY_ACK, policy });
      this.onStatusMessage?.('Transfer policy negotiated successfully.');
      return;
    }
    if (message.type === MSG_TYPE.SESSION_POLICY_ACK) {
      const policy = this.validatePolicy(message.policy);
      if (!policy) {
        await this.enforceLimit('SESSION_POLICY_ACK', 'The peer acknowledged unacceptable transfer limits.');
        return;
      }
      this.markPolicyReady(policy);
      this.onStatusMessage?.('Transfer policy negotiated successfully.');
    ...
    if (message.type === MSG_TYPE.SESSION_POLICY_REJECT) {
      this.failTransfer(`Peer rejected the transfer policy: ${message.message || 'limits are unacceptable'}`, { cleanup: true, serverCancelReason: 'policy-rejected' });
      return;
    }
    if (message.type === MSG_TYPE.LIMIT_EXCEEDED) {
      this.onStatusMessage?.(`Peer stopped the transfer: ${message.message || message.violation || 'resource limit exceeded'}`);
      this.failTransfer(message.message || 'Peer reported a transfer limit violation.', { cleanup: true, serverCancelReason: 'peer-limit-exceeded' });
      this.dataChannel?.close();
      this.binaryChannel?.close();
      return;
    }
  ...
  private handleFileList(files: FileMetadata[], transferMethod?: TransferMethod): void {
    ...
    if (!Array.isArray(files) || files.length > this.getActivePolicy().MAX_FILES) { void this.enforceLimit('MAX_FILES', `The peer announced more than ${this.getActivePolicy().MAX_FILES} files.`); return; }
    let total = 0;
    for (const file of files) { const violation = this.validateFileMetadata(file, files.length, total + file.size); if (violation) { void this.enforceLimit('FILE_METADATA', violation); return; } total += file.size; }
    this.declaredFileCount = files.length;
    this.declaredTotalBytes = total;
  ...
  private handleFileStart(message: ControlMessage): void {
    ...
    const policy = this.getActivePolicy();
    if (this.receivedFiles.has(fileIndex) || this.startedFileCount >= policy.MAX_FILES) { void this.enforceLimit('MAX_FILES', `The peer started too many files.`); return; }
    if (fileSize > policy.MAX_FILE_BYTES || this.startedTotalBytes + fileSize > policy.MAX_TOTAL_BYTES) { void this.enforceLimit('FILE_METADATA', `File ${fileName} exceeds the negotiated size budget.`); return; }
    if (totalChunks > policy.MAX_CHUNKS_PER_FILE) { void this.enforceLimit('MAX_CHUNKS_PER_FILE', `File ${fileName} declares too many chunks.`); return; }
    const declaredChunkSize = message.chunkSize;
    if (typeof declaredChunkSize !== 'number' || !Number.isSafeInteger(declaredChunkSize) || declaredChunkSize <= 0 || declaredChunkSize > policy.MAX_CHUNK_SIZE) { void this.enforceLimit('MAX_CHUNK_SIZE', `File ${fileName} declares an invalid chunk size.`); return; }
    if (new TextEncoder().encode(fileName).byteLength > policy.MAX_FILE_NAME_BYTES) { void this.enforceLimit('MAX_FILE_NAME_BYTES', `File ${fileName} has oversized metadata.`); return; }
    this.startedFileCount += 1;
    this.startedTotalBytes += fileSize;
  ...
  private handleBinaryChunk(fileIndex: number, chunkIndex: number, data: ArrayBuffer): void {
    ...
    const policy = this.getActivePolicy();
    if (chunkIndex < 0 || chunkIndex >= fileInfo.totalChunks || data.byteLength > policy.MAX_CHUNK_SIZE) { void this.enforceLimit('MAX_CHUNK_SIZE', `Received invalid chunk ${chunkIndex} for file ${fileIndex}.`); return; }
    if (fileInfo.receivedChunks.has(chunkIndex)) {
      fileInfo.duplicateBytes += data.byteLength;
      if (fileInfo.duplicateBytes > policy.MAX_DUPLICATE_BYTES) { void this.enforceLimit('MAX_DUPLICATE_BYTES', `Duplicate chunks exceeded the allowed budget for file ${fileIndex}.`); }
      return;
    }
    if (this.receivedTotalBytes + data.byteLength > policy.MAX_TOTAL_BYTES || (fileInfo.storageMode === 'indexeddb' && this.indexedDbBytes + data.byteLength > policy.MAX_TOTAL_BYTES)) { void this.enforceLimit('MAX_TOTAL_BYTES', 'Received data exceeded the negotiated session budget.'); return; }

    fileInfo.receivedChunks.add(chunkIndex);
    fileInfo.bytesReceived += data.byteLength;
    this.receivedTotalBytes += data.byteLength;
    if (fileInfo.storageMode === 'indexeddb') this.indexedDbBytes += data.byteLength;
    ...
    if (fileInfo.storageMode === 'indexeddb') {
      ...
      void this.enqueueChunkWrite(fileIndex, async () => {
        ...
        if (latestFileInfo) {
          ...
          this.receivedTotalBytes = Math.max(0, this.receivedTotalBytes - chunkToStore.byteLength);
          this.indexedDbBytes = Math.max(0, this.indexedDbBytes - chunkToStore.byteLength);
        }
        this.failTransfer(
          `Failed to persist received chunk: ${error instanceof Error ? error.message : 'unknown error'}`,
          { serverCancelReason: 'chunk-persist-failed' },
  ...
  private handleChunkRequest(message: ControlMessage): void {
    ...
    const policy = this.getActivePolicy();
    const nextRound = (this.repairRoundsReceived.get(fileIndex) ?? 0) + 1;
    if (nextRound > policy.MAX_REPAIR_ROUNDS) { void this.enforceLimit('MAX_REPAIR_ROUNDS', `Peer requested too many repair rounds for file ${fileIndex}.`); return; }
    this.repairRoundsReceived.set(fileIndex, nextRound);
  ...
  cleanup(): void {
    if (this.sessionTimeoutId) { clearTimeout(this.sessionTimeoutId); this.sessionTimeoutId = null; }
    this.policyReady = false;
    this.agreedPolicy = null;
    this.policyReadyPromise = null;
    this.resolvePolicyReady = null;
    this.declaredFileCount = 0;
    this.declaredTotalBytes = 0;
    this.startedFileCount = 0;
    this.startedTotalBytes = 0;
    this.receivedTotalBytes = 0;
    this.indexedDbBytes = 0;
    this.repairRoundsSent.clear();
    this.repairRoundsReceived.clear();
    this.limitViolationSent = false;
