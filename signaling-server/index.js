...
const ROOM_TIMEOUT = Number(process.env.ROOM_TIMEOUT_MS) > 0 ? Number(process.env.ROOM_TIMEOUT_MS) : 60 * 60 * 1000; // 1 hour for long transfers
...
// Rooms only exist once their creator announces them (see 'create-room'), and
// a room that never pairs is torn down quickly — that combination is what
// keeps a short, all-numeric code safe to use as the only secret.
const ROOM_CODE_REGEX = /^[0-9]{4}$/;
const GENERIC_JOIN_ERROR = 'Unable to join this room.';
// How long an unpaired room stays joinable before it's torn down.
const PENDING_ROOM_TIMEOUT_MS = Number(process.env.PENDING_ROOM_TIMEOUT_MS) > 0
  ? Number(process.env.PENDING_ROOM_TIMEOUT_MS)
  : 3 * 60 * 1000; // 3 minutes
// Caps how many join-room calls (successful or not) a single room will accept
// before it's torn down, independent of per-IP rate limiting — this is what
// stops an attack spread across many IPs from getting unlimited guesses
// against one specific live room.
const MAX_ROOM_JOIN_ATTEMPTS = 12;
...
const EVENT_RATE_LIMITS = {
  'create-room': { windowMs: 60 * 1000, perSocket: 6, perIp: 24, maxPayloadBytes: 1024, cost: 10 },
  'join-room': { windowMs: 30 * 1000, perSocket: 5, perIp: 5, maxPayloadBytes: 1024, cost: 10 },
...
function parseRoomActionPayload(data) {
...
function expireRoomIfInactive(roomId, timeoutMs) {
  ...
  if (timeSinceActivity >= timeoutMs) {
    if (IS_PRODUCTION) {
      logInfo(`Expiring inactive room after ${Math.round(timeoutMs / 1000 / 60 * 10) / 10} minutes`);
    } else {
      logDebug(`Room ${roomId} expired after ${timeoutMs / 1000}s`);
    }
    room.participants.forEach(socketId => {
      const socket = io.sockets.sockets.get(socketId);
      if (socket) {
        socket.emit('room-expired', { room: roomId, reason: 'inactive' });
        socket.leave(roomId);
        socket.roomId = null;
        socket.role = null;
      }
...
// Arms (or re-arms) the room's expiry timer. Unpaired rooms get a short
// window (PENDING_ROOM_TIMEOUT_MS); once a second participant joins we
// re-arm with the longer ROOM_TIMEOUT to allow for long transfers.
function armRoomTimeout(room, timeoutMs) {
  if (room.expiryTimeoutHandle) {
    clearTimeout(room.expiryTimeoutHandle);
  }
  room.expiryTimeoutHandle = setTimeout(() => {
    expireRoomIfInactive(room.id, timeoutMs);
  }, timeoutMs);
}
...
function createRoom(roomId) {
  ...
  armRoomTimeout(room, PENDING_ROOM_TIMEOUT_MS);
...
io.on('connection', (socket) => {
  ...
  socket.on('create-room', (data, acknowledgement) => {
    if (!enforceAbuseProtection(socket, clientIP, 'create-room', data)) {
      return;
    }

    const payload = parseRoomActionPayload(data);
    if (!payload) {
      rejectInvalidPayload(socket, 'create-room', 'invalid-create-room-payload');
      return;
    }
    ...
    if (socket.roomId && socket.roomId !== roomId) {
      rejectInvalidPayload(socket, 'create-room', 'socket-already-joined-another-room');
      return;
    }

    if (rooms.has(roomId)) {
      // Extremely unlikely code collision (or a stale retry) - the client
      // should generate a fresh code and try again.
      socket.emit('room-code-taken', { room: roomId });
      if (typeof acknowledgement === 'function') {
        acknowledgement({ ok: false, error: 'room-code-taken' });
      }
      return;
    }

    if (!IS_PRODUCTION) {
      logDebug(`${socket.id} (${requestedRole || 'auto'}) creating room: ${roomId} from ${country}`);
    }

    const room = createRoom(roomId);
    const assignedRole = requestedRole || 'sender';
    ...
    socket.emit('turn-servers', turnServerPayload);
    if (typeof acknowledgement === 'function') {
      acknowledgement({ ok: true, ...turnServerPayload });
    }
  });
  ...
  socket.on('join-room', (data, acknowledgement) => {
    ...
    const room = rooms.get(roomId);
    if (!room) {
      // No live room for this code - could be a wrong guess or an expired
      // room. Same generic response either way so guessing can't tell them apart.
      rejectInvalidPayload(socket, 'join-room', GENERIC_JOIN_ERROR);
      return;
    }

    const isExistingSocket = room.participants.has(socket.id);

    if (!isExistingSocket) {
      room.joinAttempts = (room.joinAttempts || 0) + 1;
      if (room.joinAttempts > MAX_ROOM_JOIN_ATTEMPTS) {
        // Too many distinct join attempts against this specific code - shut
        // it down rather than let an attacker keep guessing against a live
        // room, even if they're spreading attempts across many IPs.
        room.participants.forEach(socketId => {
          const participantSocket = io.sockets.sockets.get(socketId);
          if (participantSocket) {
            participantSocket.emit('room-expired', { room: roomId, reason: 'too-many-attempts' });
            participantSocket.leave(roomId);
            participantSocket.roomId = null;
            participantSocket.role = null;
          }
        });
        cleanup(roomId);
        rejectInvalidPayload(socket, 'join-room', GENERIC_JOIN_ERROR);
    ...
    const isReceiverJoin = requestedRole === 'receiver' || (!requestedRole && !room.participantDetails.has(socket.id));
    if (isReceiverJoin && room.receiverJoined && !isExistingSocket) {
      rejectInvalidPayload(socket, 'join-room', GENERIC_JOIN_ERROR);
      return;
    }
    ...
    if (room.participants.size >= MAX_ROOM_SIZE && !isExistingSocket) {
    ...
    room.participants.add(socket.id);
    if (assignedRole === 'receiver' && !existingParticipant) {
      room.receiverJoined = true;
    }
    room.participantDetails.set(socket.id, {
      role: assignedRole,
    ...
    // Now paired — swap the short unpaired-code window for the full
    // transfer-duration timeout.
    if (isNewParticipant && room.participants.size === MAX_ROOM_SIZE) {
      armRoomTimeout(room, ROOM_TIMEOUT);
    }
...
function startSignalingServer(port = PORT) {
...
module.exports = {
  ...
  cleanup,
  resetAbuseCounters: () => {
    socketAbuseCounters.clear();
    ipAbuseCounters.clear();
  },
};
